...
class Reporter {
    constructor (emitter, options, collectionRunOptions) {
        ...
        emitter.on('assertion', this.addAssertion.bind(this));
    ...
    onBeforeRequest (err, result) {
        ...
        const isEndRequest = name.match(/\w{4}\d{3}.*End/)
        if (!name || isEndRequest) {
            return;
        }
    ...
    // Adds each pm.test assertions as separate RP tests
    addAssertion (error, testAssertion) {
        const testObj = this.collectionMap.get(testAssertion.cursor.ref);
        if (!testObj) {
            return;
        }
        const stepName = testAssertion.assertion;
        const codeRefTitle = `${this.collectionRunOptions.collection.name}/${testAssertion.item.name}/${stepName}`;
        const parameters = utils.getParameters(this.collectionRunOptions.iterationData, testAssertion.cursor.iteration);
        const codeRef = utils.getCodeRef(this.collectionPath, codeRefTitle);
        ...
        var currentStep = stepObj.tempId

        const actualError = error || testAssertion.error;
        if (actualError) {
            // Logs error message for the failed steps
            this.logMessage(currentStep, actualError.message, 'ERROR');
        }

        this.client
            .finishTestItem(currentStep, {
                status: actualError ? TestStatus.FAILED : TestStatus.PASSED
            })
    ...
    finishAllSteps (err, testResult) {
        ...
        var requestIsRepeated = testResult.executions[0].result.return.nextRequest === testResult.item.id 
        if (requestIsRepeated) {
            // Skips tests for repeated request
            this.passAllSteps(testObj);
        }
